...
# Oppure, usando le fuzioni di rstatix:
bysubj %>% 
  group_by(group) |> 
  rstatix::get_summary_stats(bdi, type = "common")
```
:::
...
```{r}
sum((bysubj$bdi - mean(bysubj$bdi))^2) / length(bysubj$bdi)

# ovvero
var(bysubj$bdi) * (length(bysubj$bdi) - 1) / length(bysubj$bdi)
```
:::
...
```{r}
# Con la seguente istruzione rendo i risultati riproducibili.
set.seed(123)

# Definisco la funzione `my_var()` per il calcolo della varianza quale statistica descrittiva:
my_var <- function(x) {
  sum((x - mean(x))^2) / length(x)
}

# Decido di ripetere il processo di campionamento 100,000 volte.
n_rep <- 1e5

# Defisco un vettore di 100,000 elementi, ciascuno inizializzato come `NA`:
var_descr <- rep(NA, n_rep)

# Definisco l'ampiezza campionaria:
n <- 10

# Estraggo un campione di 10 osservazioni dalla popolazione del quoziente di intelligenza (per definizione avente media di 100 e deviazione standard di $15^2$). Chiamo `x` tale campione casuale di 10 osservazioni estratte a caso dalla popolazione del QI. Per tale campione calcolo la varianza quale statistica descrittiva e salvo il valore così ottenuto nell'elemento $i$-esimo del vettore `var_descr`.
for(i in 1:n_rep) {
  x <- rnorm(n, 100, 15)
  var_descr[i] <- my_var(x)
}

# Calcolo poi la media dei 100,000 valori così ottenuti:
mean(var_descr)
```
...
Di default, la funzione R `var()` usa $n-1$ al denominatore.
:::
...
```{r}
mean(abs(bysubj$bdi - mean(bysubj$bdi)))
```
:::
